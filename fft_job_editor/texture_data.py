...
def load_any_image_array(path: Path):
    """
    Loads any common image format (png/jpg/bmp/webp/tga/gif, or dds) as an
    RGBA uint8 numpy array. For a REPLACEMENT image the user provides, not
    for existing game .tex files - those need load_game_texture_preview
    below (tex-conv first).
    """
    import numpy as np
...
def save_array_as_png(arr, dest_path: Path) -> None:
    """
    Saves a full-resolution RGBA uint8 array (as returned by
    ...
    Image.fromarray(arr, "RGBA").save(str(dest_path), "PNG")
...
def load_game_texture_preview(path: Path, cli_path: Optional[Path], cache_dir: Path):
    ...
    if ext == ".tga":
        return load_any_image_array(path)
    ...
    if ext == ".tex":
        ...
        return load_any_image_array(dds_path)
...
def apply_seam_fix(arr):
    """
    Returns (fixed_array, n_promoted). Input must be an RGBA uint8 array.
    ...
    promotes a few alpha values per 4x4 block from 0 to 4 (still
    imperceptibly transparent) so the encoder doesn't leave a solid black
    seam around the portrait in-game.
    """
    import numpy as np
...
def stage_texture_replacement(
    ...
    arr = load_any_image_array(source_image_path)
    if face:
        arr, _ = apply_seam_fix(arr)
    ...
    if target_ext == ".tex":
        ...
        Image.fromarray(arr, "RGBA").save(str(staged_png))
