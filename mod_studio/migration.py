...
SECTION_JOBS = "Jobs"
SECTION_JOB_COMMANDS = "Job Commands"

# Which review section each registered per-language nxd table reports under.
# Encounters covers CharaName because unit names are part of an encounter,
# not a table a person goes looking for on its own.
NXD_SECTIONS = {
    "ability": SECTION_ABILITIES,
    "item": SECTION_ITEMS,
    "chara_name": SECTION_ENCOUNTERS,
    "poach": SECTION_POACHING,
    "job": SECTION_JOBS,
    "job_command": SECTION_JOB_COMMANDS,
}


def section_for(key: str) -> str:
    """
    Which review section a table's changes report under.

    A lookup, then a fallback - NOT a lookup alone. `NXD_SECTIONS` names
    the six tables that have a curated tab, because their review section
    should match the tab a person would go to. The other 47 registered
    tables are derived from bundled layouts and have no tab, so they
    report under their own name.

    This was a bare `NXD_SECTIONS[key]` inside a loop over the registry.
    The loop's own comment said it was built from the registry "so a newly
    registered table is reviewed on a game update without this function
    being edited" - and the very next line required exactly that edit,
    raising `KeyError` on the first table the map had not been told about.
    A registry-driven loop with a hand-written lookup inside it is not
    registry-driven.
    """
    if key in NXD_SECTIONS:
        return NXD_SECTIONS[key]
    from . import nxd_data
    return nxd_data.spec_for(key).label
...
class MigrationPlan:
    ...
    def sections(self) -> list:
        ...
        order = [SECTION_ABILITIES, SECTION_ITEMS, SECTION_JOBS,
                 SECTION_JOB_COMMANDS,
                 SECTION_POACHING, SECTION_ENCOUNTERS]
...
def build_plan(base_sqlite: Path, mod_sqlite: Path, new_sqlite: Path,
    ...
    # Built from the registry rather than listed by hand, so a newly
    # registered table is reviewed on a game update without this function
    # being edited. A table missing from here would migrate silently
    # wrong: its edits would survive into a version whose rows had moved.
    per_language = [
        (section_for(key), key,
         lambda db, lang, _k=key: nxd_data.read_nxd_table(db, _k, lang),
         intent.edits_for(key), update.edits_for(key))
        for key in nxd_data.ALL_NXD_SPECS
    ]
    for section, table_key, reader, mine, theirs in per_language:
        table_map = {lang: nxd_data.spec_for(table_key).table(lang)
...
MODELLED_NXD_FILENAMES = frozenset(
    ...
    + c.NXD_ALL_JOB_FILENAMES + c.NXD_ALL_JOBCOMMAND_FILENAMES
)
...
def apply_plan(plan: MigrationPlan) -> nxd_data.RecoveredNxdEdits:
    ...
    # (section, table prefix) -> the recovered-edits dict to write into.
    # Registry-built, so the pair for a new table cannot be forgotten - and
    # a forgotten pair here does not raise, it silently drops every change
    # for that table when a migration is applied.
    language_targets = {
        (section_for(key), spec.table_prefix): result.edits_for(key)
...
def score_baseline(candidate_sqlite: Path, mod_sqlite: Path) -> int:
    ...
    total = 0
    # Every registered table. This total decides which vanilla version a
    # mod was built against, so a table left out of the sum makes the
    # baseline guess worse without anything looking wrong.
    for key in nxd_data.ALL_NXD_SPECS:
        for rows in edits.edits_for(key).values():
